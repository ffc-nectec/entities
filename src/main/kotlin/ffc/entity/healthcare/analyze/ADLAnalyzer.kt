    ...
    override fun analyzeFrom(service: Service): HealthIssue? {
        return if (service is SpecialPP) {
            when (service.ppType.id) {
                "1B1280" -> HealthProblem(forIssue, service, Severity.OK)
                "1B1281" -> HealthProblem(forIssue, service, Severity.MID)
                "1B1282" -> HealthProblem(forIssue, service, Severity.VERY_HI)
                else -> null
            }
        } else null
    }
