    ...
    override fun analyzeFrom(service: Service): HealthIssue? {
        return if (service is SpecialPP) {
            when (service.ppType.id) {
                "1B0282", "1B0280" -> HealthProblem(forIssue, service, Severity.OK)
                "1B0281" -> HealthProblem(forIssue, service, Severity.UNDEFINED)
                "1B0283" -> HealthProblem(forIssue, service, Severity.LOW)
                "1B0284" -> HealthProblem(forIssue, service, Severity.MID)
                "1B0285" -> HealthProblem(forIssue, service, Severity.VERY_HI)
                else -> null
            }
        } else null
    }
